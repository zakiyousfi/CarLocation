...
export class ReservationsComponent implements OnInit {
  ...
  updateField(fieldName: string) {
    const fieldValue = this.reserverForm.value[fieldName];
    ...
    this.reserverForm.value[fieldName] = newValue;

    console.log('After:', this.reserverForm.value[fieldName]);
}
Chauffeur() {
  this.updateField('chauffeur');
}
GPS() {
  this.updateField('GPS');
}

SiegeBebe() {
  this.updateField('SiegeBebe');
}
reserver(){
  let user = JSON.parse(sessionStorage.getItem("user") || "[]")
  this.reserverForm.value.userId = user.id
  this.reserverForm.value.carId = this.car.id
  this.reserverForm.value.img = this.car.img
  this.reserverForm.value.name = this.car.name
  this.reserverForm.value.prix = this.car.prix
  this.reserverForm.value.Category = this.car.Category
  let reservations = JSON.parse(localStorage.getItem("reservations") || "[]")
  let id = this.generateId(reservations)
  this.reserverForm.value.id = id
  reservations.push(this.reserverForm.value)
